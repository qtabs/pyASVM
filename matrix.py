# Source: http://code.activestate.com/recipes/578131-a-simple-matrix-class/
...
class Matrix(object):
    ...
    def getSlice(self, rangeRows, rangeCols):
        matrixRows = []
        rows = [self.rows[i] for i in rangeRows]
        for row in rows:
            items = [row[j] for j in rangeCols]
            matrixRows.append(items)
        return Matrix.fromList(matrixRows)
    ...
    @classmethod
    def makeOnes(cls, m, n):
        """ Make a ones-matrix of rank (mxn) """
    ...
    def readGrid(cls, fname):
        ...
        for line in open(fname).readlines():
            if line != '':  # skip empty lines at the end of the file
    ...
    def getNorm(self):
        """ returns the norm of a vector if it is a vector"""
        if (self.m-1) * (self.n-1) != 0:
            raise MatrixError, "This function only consider vectors!"
...
def sprod(scalar, matrix):  # scalar * matrix product
    mlist = [[elem*scalar for elem in row] for row in matrix.rows]
    return Matrix.fromList(mlist)
...
class MatrixTests(unittest.TestCase):
    ...
    def testSProd(self):
        m1 = Matrix.fromList([[1, 2, 3], [4, 5, 6]])
        self.assertTrue(sprod(2,m1) == Matrix.fromList([[2, 4, 6], [8, 10, 12]]))

    def testGetSlice(self):
        m1 = Matrix.fromList([[1, 2, 3], [4, 5, 6]])
        self.assertTrue(m1.getSlice([1],[0,2]) == Matrix.fromList([[4, 6]]))
        self.assertTrue(m1.getSlice(range(0,2),range(1,3)) == Matrix.fromList([[2, 3], [5, 6]]))
    ...
    def testNorm(self):
        ...
        v1 = Matrix.fromList([[1, 2, 2, 3]])
        v2 = Matrix.fromList([[1],[2],[3],[4]])

        n1 = v1.getNorm()
        n2 = v2.getNorm()

        self.assertTrue(n1==sqrt(18) and n2 ==sqrt(30))
